...
function ProductosContent() {
  ...
  const [precioMin, setPrecioMin] = useState(() => {
    const v = searchParams.get('precioMin');
    return v !== null ? Number(v) : null;
  });
  const [precioMax, setPrecioMax] = useState(() => {
    const v = searchParams.get('precioMax');
    return v !== null ? Number(v) : null;
  });
  ...
  // Valores por defecto — sin pisar lo que ya vino de la URL (ej. al volver
  // desde la ficha de producto con filtros aplicados).
  useEffect(() => {
    setCatalogoMin(0);
    setCatalogoMax(500000);
    setPrecioMin(prev => prev ?? 0);
    setPrecioMax(prev => prev ?? 500000);
  }, []);
  ...
  // Reflejar los filtros activos en la URL (sin agregar entradas nuevas al
  // historial) para que "Volver" desde un producto restaure la búsqueda,
  // categoría, orden, rango de precio y página tal como estaban.
  useEffect(() => {
    const params = new URLSearchParams();
    if (busqueda)              params.set('busqueda',  busqueda);
    if (categoriaSeleccionada) params.set('categoria', categoriaSeleccionada);
    if (ordenar)               params.set('ordenar',   ordenar);
    if (precioMin !== null && precioMax !== null && (precioMin > catalogoMin || precioMax < catalogoMax)) {
  ...
  }, [busqueda, categoriaSeleccionada, ordenar, precioMin, precioMax, catalogoMin, catalogoMax, page, router]);
  ...
  // No resetear la página en el primer render: si venimos de la URL con
  // ?page=3, queremos respetarlo en vez de volver siempre a la página 1.
  useEffect(() => {
    if (primerRender.current) { primerRender.current = false; return; }
    setPage(1);
  }, [busqueda, categoriaSeleccionada, ordenar, precioMin, precioMax]);
